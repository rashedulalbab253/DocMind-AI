...
class MilvusVectorDB:
    ...
    def _initialize_client(self):
        try:
            # Try to use local milvus-lite if available, fallback to memory protocol
            try:
                ...
                logger.warning(f"Milvus-lite not available, using in-memory store: {lite_error}")
                # Use in-memory ephemeral connection
                self.client = MilvusClient(uri="memory://milvus")
                logger.info("Using in-memory Milvus client")
